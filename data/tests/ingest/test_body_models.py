...
class TestDamitParsers:
    ...
    def test_parse_spin_magnitude_disambiguation(self, tmp_path):
        src = tmp_path / "spin.txt"
        # λ β P on line 1; JD0 then φ0 on line 2.
        src.write_text("35.0 -60.0 7.3\n2451800.5 210.0\n")
        spin = _parse_spin(src)
        assert spin["jd0"] == 2451800.5
        assert spin["phi0"] == 210.0
        assert spin["lambda"] == 35.0
