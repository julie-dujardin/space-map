...
Per-object the export wants ONE image per derivative tree, by quality
signal rather than discovery order. This module groups direct candidates
into their ``derived_from``/``other_versions`` connected components and
scores each tree's members for the caller to pick from.
...
wikis chose for THIS object wins, globalusage breaking remaining ties:

1. Assessment (featured > quality > valued > none)
2. Pageimage count for this object (langs whose pageimage[Q] == this file)
3. Globalusage entry count (saturates at the API's gulimit cap — fine,
   saturation already means "very popular")
...
def tree_components(
    ...
    Two candidates share a component iff their forward-BFS walks reach a
    common file — catching siblings that share a parent neither is a
    parent of, which pure forward-walking would miss (metadata only
    stores child->parent edges).

    Components and their members preserve ``direct_candidates`` order.
    """
...
def best_in_tree(
    ...
    ``component`` is direct candidates known to share a tree (e.g. from
    :func:`tree_components`); all forward-reachable files join the pool,
    ties breaking on ``(assessment, pageimage_count, globalusage_count,
    -discovery_order)``.

    Tree-only members (not in ``discovery_order_of``) score at order=∞,
    winning only on a higher assessment/pageimage/globalusage.
    """
    members: set[str] = set()
