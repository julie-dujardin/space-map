...
Always-loaded, like `atmospheres.json`: the panel needs every vehicle at once.
Δv is derived from the rocket equation (value + inputs, so the panel can show
either); C3 curves are thinned from the downloaded launch-performance files so
they fit the boot-time payload budget.

Names live separately in `v1/spacecraft/<lang>.json`, one bundle per locale —
a name is the one part that differs per reader, so it's not worth shipping
twelve locales of it in every download. Labels are Wikidata's, not hand-translated.
"""
...
def _curve_payload(curve: C3Curve) -> dict:
    ...
    entry: dict = {
        ...
        "source": curve.source,
        # Distinguishes "cannot fly this" from "nobody published that far".
...
def _entry(craft: Spacecraft) -> dict:
    ...
    # Kept separate, not pre-joined into the name: each is its own message key
    # so "expendable" localizes and "Star 48" (a part number) doesn't.
    if craft.variant:
        entry["variant"] = list(craft.variant)
    if craft.power:
        entry["power"] = craft.power
    # Fiction only, where the work makes propellant a non-issue.
    ...
    # Shipped alongside its inputs so the panel can show the working; absent
    # where an input is, since an unverifiable Δv is worse than none.
...
def build_name_bundles(cache: WikidataEntityCache) -> dict[str, dict[str, dict]]:
    ...
    a row labelled by its slug is worse than one in the wrong language. Craft
    with no Wikidata item are absent; the frontend hand-authors those names.
    """
    bundles: dict[str, dict[str, dict]] = {lang: {} for lang in LANGUAGES}
    fell_back: dict[str, int] = {}
    unnamed: list[str] = []
    for craft in CATALOGUE.values():
        # Same cut as the catalogue: a vehicle that does not ship needs no name.
        ...
        for lang in LANGUAGES:
            ...
            # Wikidata's one-liner, e.g. "American super heavy-lift launch vehicle".
...
def build_spacecraft() -> dict:
    ...
    Entries the solver could never judge are not exported: a row of "no
    published figure" answers tells the reader nothing a missing row doesn't.
    """
    vehicles = []
    for craft in CATALOGUE.values():
        if not solver_can_judge(craft):
...
def write_spacecraft(out_dir: Path, cache: WikidataEntityCache | None = None) -> None:
    ...
    # Non-launcher, no Δv: worth a line so a figure that turns up gets noticed.
