...
The activity, atmosphere, interior and temperature registries are separate
tuples that agree on their first four fields, so a body's credit line looks the
same whichever of them a number came from. `contribution` stays behind: it is
the credits page's full sentence, where a footer under a panel has room for the
...
Reference = (
    ActivityReference | AtmosphereReference | InteriorReference | TemperatureReference
)
