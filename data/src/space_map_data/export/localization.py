...
# base locale are pruned. A hand-written key must never start with one of
# these — the prune claims the whole namespace and would delete it.
GENERATED_PREFIXES = (
    "unit_name_",
    "unit_symbol_",
    "property_name_",
...
def _collect_feature_type_labels(
    ...
    Keyed by the type's slug stem, shared with its ``ft-`` collection page
    name — no separate ``group_name_ft-*`` twin. Locales missing a Wikidata
    label/description are left out so Paraglide's baseLocale fallback fills
    them in; baseLocale itself is always backfilled from ``FEATURE_TYPES``.
    """
...
def _collect_group_name_labels(
    ...
    Wikidata entity. Country, orbit-class and feature-type groups are
    skipped: each resolves its name from elsewhere already (ISO code,
    ``orbit_class_<NAME>``, ``feature_type_label_<stem>``).
    """
    result: dict[str, dict[str, str]] = {lang: {} for lang in LANGUAGES}
    base = result[BASE_LOCALE]
...
def _merge_all_locales(
    ...
    Base locale defines which keys are still live; anything else belongs to
    a removed group/unit/property. It also seeds the values other locales
    are compared against — an identical translation is dropped so
    Paraglide's fallback serves it instead.
    """
...
def _merge_into_file(
    ...
    *live_keys*. When *base_values* is given, a generated value identical to
    the base locale is omitted so Paraglide's fallback serves it instead.
    """
