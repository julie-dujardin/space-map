...
def moon_orbit(moon: SBDBMoon | AsterSatMoon | None) -> dict[str, float]:
    ...
    for the full Keplerian set by membership. SBDB satellite orbits ship a
    period but never a mean motion, so n is derived when it's missing.
    """
    if moon is None:
        return {}
    if isinstance(moon, AsterSatMoon):
        out: dict[str, float] = {
            "epoch_jd": moon.epoch_mjd + _MJD_OFFSET,
            "a": moon.a_km / _AU_KM,
            "e": moon.e,
            "i": moon.i,
            "om": moon.om,
        ...
        out = {}
        if moon.epoch_jd is not None:
            out["epoch_jd"] = moon.epoch_jd
        if moon.a_km is not None:
            out["a"] = moon.a_km / _AU_KM
        for key in ("e", "i", "om", "w", "ma"):
            value = getattr(moon, key)
            if value is not None:
                out[key] = value
        if moon.n is not None:
            out["n"] = moon.n
        elif moon.per_h:
            out["n"] = 360.0 * _HOURS_PER_DAY / moon.per_h
        frame = moon.frame

    if is_equatorial(frame) and {"i", "om"} <= out.keys():
