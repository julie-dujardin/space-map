...
flowchart LR
    ...
    NAIF_PROBES[(NAIF + ESA + NAIF-PDS<br/><i>per-mission spacecraft SPKs</i>)]
    ...
    subgraph DL["DOWNLOAD_DIR — raw + pre-fit artefacts"]
        ...
        D_PROBES["probes downloader<br/><i>per-mission SPKs from NAIF operational tree,<br/>ESA mirror, NAIF PDS3/PDS4 archives;<br/>MISSION_INCLUDE regex whitelist + stationary-debris skip;<br/>writes _index.json per mission</i>"]
        ...
        F_PROBES[/"spice/kernels/missions/&lt;MISSION&gt;/*.bsp + _index.json"/]
    ...
    subgraph ING["INGEST — parse, cross-reference, deduplicate"]
        ...
        I_PROBES["probes ingest<br/><i>walk missions/*/_index.json, skip landed probes,<br/>compute inception MJD via _coverage(),<br/>assign stable probe_id (cached on disk),<br/>create Object rows id_type=probe</i>"]
    ...
    subgraph EXP["EXPORT — partition, pack, localize"]
        ...
        E_LOC["localization (i18n)<br/><i>Wikidata labels for units / properties /<br/>IAU feature types, merge into frontend messages,<br/>strip old autogenerated keys, missing locales fall<br/>back to baseLocale at Paraglide compile time</i>"]
        ...
        OUT_POS_PROBES[/"position/probes/{zone}/{chunk}.bin.gz"/]
