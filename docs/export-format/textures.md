...
- `distribution` — who may serve the map, omitted for the common case of one anyone may. `"non-commercial"` is a licence that forbids commercial reuse, which an embed opts into; `"site-only"` is cleared for spacemap.co alone and no embed may ask for it. Mirrored into the `systems/{bary}.json` texture block, where the renderer reads it before requesting a tier. Attribution does not depend on it — every map is credited, whether or not its licence compels it.
- `alternates` — in `systems/{bary}.json` and the object bundle, beside `texture`: the maps ranked below the best one, best first. Each carries its own `id` and `tiers` because it is a separate bundle at `textures/<body>_alt-<variant>/`, plus the same credit fields. A viewer that may not serve the best map walks this list instead of drawing an untextured sphere, and credits whichever one it drew. Absent for the bodies with only one map, which is most of them.
