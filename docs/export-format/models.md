...
- `true_scale` — axis-aligned mesh bounds in **km**, read from the raw pre-Meshopt POSITION accessor (Meshopt quantises positions, so the shipped GLB's min/max are quantisation range, not km). `max_extent_km` sizes the camera framing; the frontend renders the mesh at literal km so the shape is true-to-scale.
