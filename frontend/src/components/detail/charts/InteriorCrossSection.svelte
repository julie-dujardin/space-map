<script lang="ts">
	...
	interface Props {
		...
		/** Height the atmosphere strip draws to, in km. Omitted on a body whose
	...
	// The viewBox matches the drawer's content width so the label sizes below are
	// the sizes they actually render at; scaling a 320-wide chart into 264px
	// shrank 10px text to 8.
	const W = 264;
	const H = 190;
	...
	function reading(i: number): string | null {
		...
		return ltrIsolate(
			formatTemperatureRange(
				{ value: value.lowK, unit: 'kelvin' },
				{ value: value.highK, unit: 'kelvin' }
			)
		);
	}
