<!--
  The gallery itself: one section per world, one card per traverse on it. The
  page around it draws before this does, so it takes the payload already
  resolved.
-->
<script lang="ts">
	...
	import type { GalleryData } from '../../routes/view/+page';
	...
	const bodies = $derived(gallery.bodies);
	...
	const summary = $derived.by(() => {
		...
		return m.panorama_gallery_summary({
			panoramas: m.panorama_gallery_summary_panoramas(n(gallery.summary.panoramas)),
			probes: m.panorama_gallery_summary_probes(n(gallery.summary.probes)),
			worlds: m.panorama_gallery_summary_worlds(n(gallery.summary.worlds))
		});
	});
</script>
